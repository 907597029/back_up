...
def GetXiangceUrl(siteUrl):
    ...
    try:
        ...
        # req=request.Request(siteUrl,headers=headers.items(1))
        html = urlopen(req)
...
def GetXiangcePageUrl(xiangceurl):
    ...
    try:
        ...
        # req=request.Request(xiangceurl,headers=headers.items(2))
        html = urlopen(req)
        bsObj = BeautifulSoup(html.read(), "html.parser")
        html.close()
    except UnicodeDecodeError as e:
    ...
    if bsObj != "":
        for pagelist in bsObj.findAll("div", {"class": "pagenavi"}):
            for link in pagelist.findAll("a", href=re.compile("http://www.mzitu.com/([0-9]+)/([0-9]+)*$")):  # 正则匹配相册链接
...
def GetPageImageUrl(XiangCePageUrl, path, replaceurl):
    ...
    try:
        ...
        # req=request.Request(XiangCePageUrl,headers=)
        html = urlopen(req)
        bsObj = BeautifulSoup(html.read(), "html.parser")
        html.close()
            ...
            for ImageUrllist in templist.findAll("img"):
                ...
                if len(ImageNameNew) != 0:
                    ...
                    try:
                        ...
                        urlretrieve(Imageurl, str(path) + "\\" + "".join(ImageNameNew) + str(bianhao) + ".jpg",
                                    jindu)  # 下载图片，并调用回调函数jindu()显示下载进度
